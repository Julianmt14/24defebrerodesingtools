...
const DesignStudio = () => {
  ...
  const fetchSavedDesigns = async () => {
    try {
      ...
      const filtered = (response.data || []).filter(d => d.design_type === 'beam_detailing');
      setSavedDesigns(filtered);
    } catch (error) {
      console.error('Error fetching designs:', error);
      toast.error('Error al cargar vigas guardadas');
    } finally {
  ...
  const deleteDesign = async (id) => {
    try {
      ...
      setSavedDesigns((prev) => prev.filter((calc) => calc.id !== id));
      toast.success('Despiece eliminado', { style: { background: '#0f172a', color: '#fff', border: '1px solid #1e293b' } });
    } catch (error) {
      console.error('Error deleting design:', error);
      toast.error('Error al eliminar el despiece');
    }
  };
  ...
  const handleLoadDesign = async (design) => {
    try {
      const response = await apiClient.get(`/api/v1/tools/despiece/designs/${design.id}`);
      const despieceData = response.data;
      const settingsData = design.settings || {};

      const combined = { ...despieceData, ...settingsData };
      const parsedSpans = (combined.span_geometries || []).map(span => ({
        clear_span_between_supports_m: Number(span.clear_span_between_supports_m || 0),
        section_base_cm: Number(span.base_cm || span.section_base_cm || 0),
        section_height_cm: Number(span.height_cm || span.section_height_cm || 0)
      }));

      let axisLabels = [];
      if (combined.axis_numbering) {
        axisLabels = combined.axis_numbering.split('·').map(s => s.trim());
      }
      const parsedAxis = (combined.support_widths_cm || []).map((w, i) => ({
        label: axisLabels[i] || '',
        support_width_cm: Number(w || 0)
      }));

      const topBars = settingsData.top_bars_config || [];
      const bottomBars = settingsData.bottom_bars_config || [];

      const parsedSegments = (combined.segment_reinforcements || []).map(seg => ({
        span_indexes: seg.span_indexes || [],
        top_quantity: seg.top_rebar?.quantity || undefined,
        top_diameter: seg.top_rebar?.diameter || undefined,
        bottom_quantity: seg.bottom_rebar?.quantity || undefined,
        bottom_diameter: seg.bottom_rebar?.diameter || undefined,
      }));

      const formValues = {
        project_name: combined.project_name || '',
        beam_label: combined.beam_label || '',
        element_identifier: combined.element_identifier || '',
        element_level: Number(combined.element_level || 0),
        element_quantity: Number(combined.element_quantity || 1),
        top_bars_config: topBars,
        bottom_bars_config: bottomBars,
        segment_reinforcements: parsedSegments,
        max_rebar_length_m: combined.max_rebar_length_m || '9m',
        has_initial_cantilever: Boolean(combined.has_cantilevers),
        has_final_cantilever: Boolean(combined.has_cantilevers),
        span_geometries: parsedSpans,
        hook_type: combined.hook_type || '135',
        cover_cm: Number(combined.cover_cm || 4),
        axis_supports: parsedAxis,
        span_count: parsedSpans.length || 1,
        stirrups_config: combined.stirrups_config || [],
        energy_dissipation_class: combined.energy_dissipation_class || 'DES',
        concrete_strength: combined.concrete_strength || '21 MPa (3000 psi)',
        reinforcement: combined.reinforcement || '420 MPa (Grado 60)',
        beam_total_length_m: Number(combined.beam_total_length_m || 0)
      };

      reset(formValues);
      setLastDesign(design);

      if (combined.detailing_results && combined.detailing_computed) {
        setDetailingResults(combined.detailing_results);
      ...
      toast.success('Despiece cargado correctamente', { style: { background: '#0f172a', color: '#fff', border: '1px solid #1e293b' } });
    } catch (error) {
      console.error('Error loading design:', error);
      toast.error('Error al cargar el despiece');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050b16] text-slate-100 font-[Inter]">
      <header className="border-b border-slate-800 bg-[#0f172a] px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <div className="flex bg-[#050b16] border border-slate-700 rounded-2xl p-1 text-[11px] font-bold uppercase tracking-[0.3em]">
            {energyOptions.map((option) => (
              <button
                ...
                className={`px-3 py-1 rounded-xl transition-colors ${energyClass === option ? 'bg-primary text-white' : 'text-slate-400'
                  }`}
              >
                {option}
              </button>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#1e293b] hover:bg-[#334155] border border-slate-700 text-slate-300 py-1.5 px-4 rounded-xl text-sm font-semibold transition-colors ml-4"
          >
            <span className="material-symbols-outlined text-[18px]">history</span>
            Ver Guardados
          </button>
          <div className="flex gap-1 bg-slate-900/40 border border-slate-700 rounded-2xl px-4 py-2 ml-2">
      ...
      {/* Saved Designs Modal */}
      {showSavedModal && (
        <div className="fixed inset-0 bg-[#050b16]/90 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-[#0c1326] border border-slate-700 w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              {isLoadingSaved ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-slate-500 gap-2">
                  ...
                  <p>No tienes ningún despiece guardado todavía.</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {savedDesigns.map((design) => (
                    <div
                      ...
                      className="bg-[#050b16] border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-primary/50 hover:bg-[#070d1c] transition-all cursor-pointer shadow-sm"
                    >
                      <div className="flex flex-col flex-1 min-w-0">
                        <span className="text-white font-semibold text-base mb-1 truncate block max-w-[250px] md:max-w-[450px] group-hover:text-primary transition-colors">
                          {design.despiece_viga?.project_name || design.settings?.project_name || 'Sin Proyecto'} - {design.title}
                        </span>
                      ...
                      <div className="flex items-center gap-4 bg-[#0c1326] p-2 px-3 rounded-lg border border-slate-800">
                        <div className="text-center">
                          <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">NSR-10</div>
                          <div className="text-slate-300 font-bold text-sm">{design.settings?.energy_dissipation_class || 'DES'}</div>
                        </div>
                        <div className="w-px h-6 bg-slate-800"></div>
                        <div className="text-center">
                          <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Luces</div>
                          <div className="text-emerald-400 font-bold text-sm">{design.settings?.span_count || 1}</div>
                        </div>
                        <div className="w-px h-6 bg-slate-800"></div>
                        <div className="text-center">
                          <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">f'c</div>
                          <div className="text-white font-bold text-sm truncate max-w-[80px]" title={design.settings?.concrete_strength}>
                            {design.settings?.concrete_strength?.split(' ')[0] || '21'}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center">
                        <button
                          ...
                          className="text-slate-500 hover:text-rose-400 bg-[#0c1326] hover:bg-rose-500/10 p-2 rounded-lg transition-colors border border-transparent hover:border-rose-500/30"
